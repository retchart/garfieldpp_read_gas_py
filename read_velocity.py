'''
Title:      read_velocity.py
Author:     Liu Lixing, PhD, 
            Tsinghua University
            liulx18@mails.tsinghua.edu.cn
Abstract:   Read the electron's velocity of the gas file generated by 
            garfield++.
'''
...
def read_gas(file):
    ...
    # parse data
    velocity = np.zeros((nE, nA, nB), dtype=float)
    ...
    for l in range(nE):
        for m in range(nA):
            for n in range(nB):
                if(tab2d):
                    velocity[l][m][n] = data[l* nA * nB + m * nB + n][0]
    ...
    return pressure, efield, velocity, diffusionl, diffusiont, alpha, alpha0, eta
...
velocity = [[]for i in range(len(gas_file))]
velocity_plot = [[]for i in range(len(gas_file))]
...
for i in range(len(gas_file)):
    (pressure[i], efield[i], velocity[i], diffusionl[i], diffusiont[i], alpha[i], alpha0[i], eta[i]) = \
        read_gas(gas_file[i])

    # convert data from torr to atm
    efield_plot[i] = efield[i] * 760.
    velocity_plot[i] = velocity[i][:,0,0]
    # plot data
    plt.plot(efield_plot[i], velocity_plot[i], "-o", label="%s" %(gas_file[i]))
plt.xlabel("Reduced electric field (E/cm/atm)")
plt.xscale("log")
plt.ylabel("Drift velocity (cm/us)")
plt.yscale("linear")
plt.ylim(0, 35)
plt.legend()
plt.show()
