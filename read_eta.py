...
Abstract:   Read the electron's attanchment of the gas file generated by 
            garfield++.
'''
import numpy as np
...
for i in range(len(gas_file)):
    ...
    eta_plot[i] = np.exp(eta[i][:,0,0] + math.log(760))
    # plot data
    plt.plot(efield_plot[i], eta_plot[i], "-o", label="%s" %(gas_file[i]))
plt.xlabel("Reduced electric field (E/cm/atm)")
plt.xscale("log")
plt.ylabel("attachment coefficient (1/cm)")
plt.yscale("log")
plt.ylim(1e-6, 1e2)
plt.legend()
plt.show()
